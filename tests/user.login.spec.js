...
test ('create new account', async ({page}) => {
    ...
    await page.getByPlaceholder('Name').fill('auto@test.za');
    ...
    await page.locator('form').filter({ hasText: 'Signup' }).getByPlaceholder('Email Address').fill('auto@test.za');
    await page.getByPlaceholder('Name').isVisible();
 ...
 test('login with an existing account', async ({ page }) => { 
      

await page.getByRole('link', { name: ' Signup / Login' }).click();
await page.locator('form').filter({ hasText: 'Login' }).getByPlaceholder('Email Address').click();
await page.locator('form').filter({ hasText: 'Login' }).getByPlaceholder('Email Address').fill('auto@test.za');
await page.getByPlaceholder('Password').isVisible();
await page.getByPlaceholder('Password').fill('Test123');
await page.getByRole('button', { name: 'Login' }).click();
await page.getByText('Logged in as John Tester').isVisible();
await page.getByRole('link', { name: ' Delete Account' }).isVisible();
await page.getByRole('link', { name: ' Delete Account' }).click();
await //page.frameLocator('iframe[name="aswift_1"]').frameLocator('iframe[name="ad_iframe"]').getByLabel('Close ad').click();
